...
test('approved NAICS classifications are published without unverified PSC codes', () => {
  ...
  assert.match(home, /Federal solicitations assign NAICS codes by requirement/)
  assert.match(home, /SAM Primary NAICS/)
  assert.match(home, /Additional NAICS/)
...
test('primary business copy exists as static semantic HTML', () => {
  ...
  assert.match(home, /helps government buyers execute defined supply and procurement requirements/)
  assert.match(home, /coordinate sourcing, supplier pricing, documentation, and delivery from requirement through fulfillment/)
  ...
  assert.match(home, /Execution Controls/)
  assert.match(home, /Built for controlled contract execution\./)
  assert.match(home, /Verified Federal Registration/)
  assert.match(home, /Supplier &amp; Quote Control/)
  ...
  assert.match(home, /reviews complete procurement inquiries within one business day/)
...
test('approved capability statement is published inside contracting information', () => {
  ...
  const documentLink = home.indexOf('class="button document-resource-link"', contracting)
  ...
  assert.equal((home.match(/VANITUS Capability Statement 2026/g) || []).length, 1)
  assert.equal((home.match(/document-resource-link/g) || []).length, 1)
...
test('approved visual rhythm is integrated without changing section order', () => {
  ...
  const technology = home.indexOf('class="visual-break visual-break-technology"')
  const proof = home.indexOf('id="execution-proof-title"')
  const columns = home.indexOf('class="visual-break visual-break-columns"')
  const contracting = home.indexOf('id="contracting"')
  const fulfillment = home.indexOf('class="visual-break visual-break-fulfillment"')
  const process = home.indexOf('id="process"')
  const about = home.indexOf('id="about"')
  const contact = home.indexOf('id="contact"')
  assert.ok(capabilities < technology && technology < proof && proof < columns && columns < contracting)
  assert.ok(contracting < fulfillment && fulfillment < process && process < about && about < contact)
  assert.match(home, /vanitus-technology-640\.webp/)
  assert.match(home, /vanitus-technology-1440\.webp/)
  assert.match(home, /vanitus-fulfillment-640\.webp/)
  assert.match(home, /vanitus-fulfillment-1440\.webp/)
  ...
  assert.equal((home.match(/loading="lazy"/g) || []).length, 4)
  assert.match(home, /class="about-media" aria-hidden="true"/)
  assert.match(css, /\.visual-break \{[^}]*overflow: hidden/)
  assert.match(css, /\.visual-break-technology/)
  assert.match(css, /\.visual-break-fulfillment/)
